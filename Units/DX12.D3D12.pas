...
type
  ...
  TD3D12_VIEWPORT = record
    ...
    constructor Create(topLeftX: single; topLeftY: single; Width: single; Height: single;
      minDepth: single = D3D12_MIN_DEPTH; maxDepth: single = D3D12_MAX_DEPTH); overload;
    constructor Create(pResource: ID3D12Resource; mipSlice: UINT = 0; topLeftX: single = 0.0;
      topLeftY: single = 0.0; minDepth: single = D3D12_MIN_DEPTH; maxDepth: single = D3D12_MAX_DEPTH); overload;
  end;
  ...
  TD3D12_DEPTH_STENCIL_DESC = record
    ...
    constructor Create(depthEnable: boolean; depthWriteMask: TD3D12_DEPTH_WRITE_MASK; depthFunc: TD3D12_COMPARISON_FUNC;
      ...
      frontStencilDepthFailOp: TD3D12_STENCIL_OP; frontStencilPassOp: TD3D12_STENCIL_OP;
      frontStencilFunc: TD3D12_COMPARISON_FUNC; backStencilFailOp: TD3D12_STENCIL_OP; backStencilDepthFailOp: TD3D12_STENCIL_OP;
      backStencilPassOp: TD3D12_STENCIL_OP; backStencilFunc: TD3D12_COMPARISON_FUNC); overload;
  end;
  ...
  TD3D12_SUBRESOURCE_TILING = record
    ...
    StartTileIndexInOverallResource: UINT;
        ...
        {$ENDIF}
    procedure Init; inline;
    constructor Create(widthInTiles: UINT; heightInTiles: uint16; depthInTiles: uint16;
      startTileIndexInOverallResource: UINT);
  end;
  ...
  TD3D12_PACKED_MIP_INFO = record
    ...
    StartTileIndexInOverallResource: UINT;
        ...
        {$ENDIF}
    procedure Init; inline;
    constructor Create(numStandardMips: uint8; numPackedMips: uint8; numTilesForPackedMips: UINT;
      startTileIndexInOverallResource: UINT);
  end;
  ...
  TD3D12_SUBRESOURCE_FOOTPRINT = record
    ...
    constructor Create(format: TDXGI_FORMAT; Width: UINT; Height: UINT; depth: UINT;
  ...
  TD3D12_DESCRIPTOR_RANGE = record
    ...
    constructor Create(rangeType: TD3D12_DESCRIPTOR_RANGE_TYPE; numDescriptors: UINT;
      baseShaderRegister: UINT; registerSpace: UINT = 0;
  ...
  TD3D12_STATIC_SAMPLER_DESC = record
    ...
    constructor Create(shaderRegister: UINT; filter: TD3D12_FILTER = D3D12_FILTER_ANISOTROPIC;
      ...
      maxLOD: single = D3D12_FLOAT32_MAX; shaderVisibility: TD3D12_SHADER_VISIBILITY = D3D12_SHADER_VISIBILITY_ALL;
...
//    {$IF FPC_FULLVERSION >= 30101}
  ...
  ID3D12Device6 = interface(ID3D12Device5)
    ...
    function SetBackgroundProcessingMode(Mode: TD3D12_BACKGROUND_PROCESSING_MODE;
      MeasurementsAction: TD3D12_MEASUREMENTS_ACTION; hEventToSignalUponCompletion: THANDLE;
      out pbFurtherMeasurementsDesired: boolean): HResult; stdcall;
  ...
  ID3D12Device7 = interface(ID3D12Device6)
    ...
    function AddToStateObject(const pAddition: TD3D12_STATE_OBJECT_DESC;
      pStateObjectToGrowFrom: ID3D12StateObject; const riid: TGUID; out ppNewStateObject): HResult; stdcall;

    function CreateProtectedResourceSession1(const pDesc: TD3D12_PROTECTED_RESOURCE_SESSION_DESC1;
      const riid: TGUID; out ppSession): HResult; stdcall;
  ...
  ID3D12Device8 = interface(ID3D12Device7)
    ...
    function CreateCommittedResource2(const pHeapProperties: TD3D12_HEAP_PROPERTIES;
      HeapFlags: TD3D12_HEAP_FLAGS; const pDesc: TD3D12_RESOURCE_DESC1;
      InitialResourceState: TD3D12_RESOURCE_STATES; const pOptimizedClearValue: TD3D12_CLEAR_VALUE;
      pProtectedSession: ID3D12ProtectedResourceSession; const riidResource: TGUID; out ppvResource): HResult; stdcall;

    function CreatePlacedResource1(pHeap: ID3D12Heap; HeapOffset: uint64;
      const pDesc: TD3D12_RESOURCE_DESC1; InitialState: TD3D12_RESOURCE_STATES;
      const pOptimizedClearValue: TD3D12_CLEAR_VALUE; const riid: TGUID; out ppvResource): HResult; stdcall;

    procedure CreateSamplerFeedbackUnorderedAccessView(pTargetedResource: ID3D12Resource;
      pFeedbackResource: ID3D12Resource; DestDescriptor: TD3D12_CPU_DESCRIPTOR_HANDLE); stdcall;
    ...
    procedure GetCopyableFootprints1(const pResourceDesc: TD3D12_RESOURCE_DESC1; FirstSubresource: UINT;
      ...
      out pNumRows {NumSubresources}: PUINT; out pRowSizeInBytes {NumSubresources}: PUINT64;
      out pTotalBytes: uint64); stdcall;
  ...
  ID3D12GraphicsCommandList6 = interface(ID3D12GraphicsCommandList5)
    ...
    procedure DispatchMesh(ThreadGroupCountX: UINT; ThreadGroupCountY: UINT;
...
constructor TD3D12_ROOT_DESCRIPTOR_TABLE1.Create(numDescriptorRanges: UINT; const pDescriptorRanges: PD3D12_DESCRIPTOR_RANGE1_ARRAY);
begin
...
constructor TD3D12_RESOURCE_DESC.Tex3D(format: TDXGI_FORMAT; Width: uint64; Height: UINT; depth: uint16;
