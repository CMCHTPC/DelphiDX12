...
{ **************************************************************************
   ...
   This module defines Xbox 360 Common Controller APIs
...
TXINPUT_GAMEPAD = _XINPUT_GAMEPAD;
...
TXINPUT_VIBRATION = _XINPUT_VIBRATION;
...
TXINPUT_CAPABILITIES = _XINPUT_CAPABILITIES;
...
TXINPUT_BATTERY_INFORMATION = _XINPUT_BATTERY_INFORMATION;
...
TXINPUT_KEYSTROKE = _XINPUT_KEYSTROKE;
...
{_In_  } dwUserIndex : DWORD ;
// Receives the current state
{_Out_ } pState : PXINPUT_STATE
...
{_In_ } pVibration : PXINPUT_VIBRATION
    ) : DWORD;stdcall; external XINPUT_DLL;
...
{_Out_ } pCapabilities : PXINPUT_CAPABILITIES
    ) : DWORD;stdcall;external XINPUT_DLL;
...
{_Out_writes_opt_(*pRenderCount) } pRenderDeviceId : LPWSTR ;
// Size of render device ID string buffer (in wide-chars)
{_Inout_opt_ } pRenderCount : PUINT ;
// Windows Core Audio device ID string for capture (microphone)
{_Out_writes_opt_(*pCaptureCount) } pCaptureDeviceId : LPWSTR ;
// Size of capture device ID string buffer (in wide-chars)
{_Inout_opt_ } pCaptureCount : PUINT
    ) : DWORD;stdcall; external XINPUT_DLL;
...
{_Out_ } pBatteryInformation : PXINPUT_BATTERY_INFORMATION
    ) : DWORD;stdcall; external XINPUT_DLL;
...
{_Out_      } pKeystroke : PXINPUT_KEYSTROKE
    ) : DWORD;stdcall; external XINPUT_DLL;
...
{_Out_ } pDSoundRenderGuid : PGUID ;
// DSound device ID for capture (microphone)
{_Out_ } pDSoundCaptureGuid : PGUID
    ) : DWORD;stdcall; external XINPUT_DLL;
