...
var
    ...
    mainDescriptorHeap: array [0..frameBufferCount - 1] of ID3D12DescriptorHeap; // this heap will store the descripor to our constant buffer
    constantBufferUploadHeap: array [0..frameBufferCount - 1] of ID3D12Resource; // this is the memory on the gpu where our constant buffer will be placed.

    cbColorMultiplierData: TConstantBuffer; // this is the constant buffer data we will send to the gpu
    // (which will be placed in the resource we created above)

    cbColorMultiplierGPUAddress: array [0..frameBufferCount - 1] of pointer; // this is a pointer to the memory location we get when we map our constant buffer



    rIncrement: single = 0.002;
    gIncrement: single = 0.006;
    bIncrement: single = 0.009;
    ...
    const
        ...
        vList: array [0..3] of TVertex = (
            ...
            (pos: (x: 0.5; y: 0.5; z: 0.5); color: (r: 1.0; g: 1.0; b: 0.0; a: 1.0))
            );
    ...
    var
        ...
        rootParameters: array[0..0] of TD3D12_ROOT_PARAMETER; // only one parameter right now
    ...
    begin
        ...
        rootSignatureDesc.Init(Length(rootParameters), // we have 1 root parameter
            ...
            Ord(D3D12_ROOT_SIGNATURE_FLAG_DENY_GEOMETRY_SHADER_ROOT_ACCESS) or Ord(D3D12_ROOT_SIGNATURE_FLAG_DENY_PIXEL_SHADER_ROOT_ACCESS)));
        ...
        device.CreateCommittedResource(
            TD3DX12_HEAP_PROPERTIES.Create(D3D12_HEAP_TYPE_DEFAULT),
            D3D12_HEAP_FLAG_NONE,
            TD3DX12_RESOURCE_DESC.Tex2D(DXGI_FORMAT_D32_FLOAT, Width, Height, 1, 0, 1, 0, D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL),
            D3D12_RESOURCE_STATE_DEPTH_WRITE, @depthOptimizedClearValue,
            IID_ID3D12Resource, depthStencilBuffer
            );
        dsDescriptorHeap.SetName('Depth/Stencil Resource Heap');
        ...
        begin
            ...
            hr := device.CreateDescriptorHeap(heapDesc, IID_ID3D12DescriptorHeap, @mainDescriptorHeap[i]);
            if (FAILED(hr)) then
            begin
                Running := False;
            end;
        ...
        begin
            hr := device.CreateCommittedResource(TD3DX12_HEAP_PROPERTIES.Create(D3D12_HEAP_TYPE_UPLOAD),
                ...
                D3D12_HEAP_FLAG_NONE, // no flags
                TD3DX12_RESOURCE_DESC.Buffer(1024 * 64), // size of the resource heap. Must be a multiple of 64KB for single-textures and constant buffers
                D3D12_RESOURCE_STATE_GENERIC_READ, // will be data that is read from so we keep it in the generic read state
                nil, // we do not have use an optimized clear value for constant buffers
                IID_ID3D12Resource, constantBufferUploadHeap[i]);
            constantBufferUploadHeap[i].SetName('Constant Buffer Upload Resource Heap');


            cbvDesc.BufferLocation := constantBufferUploadHeap[i].GetGPUVirtualAddress();
            cbvDesc.SizeInBytes := (sizeof(TConstantBuffer) + 255) and not 255;    // CB size is required to be 256-byte aligned.

            lrv:= mainDescriptorHeap[i].GetCPUDescriptorHandleForHeapStart();
            device.CreateConstantBufferView(cbvDesc, lrv );

            ZeroMemory(@cbColorMultiplierData, sizeof(cbColorMultiplierData));
            cbColorMultiplierData.colorMultiplier.r := 0.5;
            cbColorMultiplierData.colorMultiplier.g := 0.5;
            cbColorMultiplierData.colorMultiplier.b := 0.5;

            readRange := TD3DX12_RANGE.Create(0, 0);    // We do not intend to read from this resource on the CPU. (End is less than or equal to begin)
            hr := constantBufferUploadHeap[i].Map(0, @readRange, cbColorMultiplierGPUAddress[i]);
            // move(cbColorMultiplierData, cbColorMultiplierGPUAddress[i], sizeof(cbColorMultiplierData));
            iTemp:=sizeof(cbColorMultiplierData);
            Windows.CopyMemory(@cbColorMultiplierGPUAddress[i], @cbColorMultiplierData, iTemp);
        end;
 ...
 (*       cbColorMultiplierData.colorMultiplier.x :=0.5;
        cbColorMultiplierData.colorMultiplier.y  :=0.5;
        cbColorMultiplierData.colorMultiplier.z  :=0.5;   *)

    (*
        if ((cbColorMultiplierData.colorMultiplier.x >= 1.0) or (cbColorMultiplierData.colorMultiplier.x <= 0.0)) then
        begin
            if cbColorMultiplierData.colorMultiplier.x >= 1.0 then
                cbColorMultiplierData.colorMultiplier.x := 1.0
            else
                cbColorMultiplierData.colorMultiplier.x := 0.0;
            rIncrement := -rIncrement;
        end;

        if ((cbColorMultiplierData.colorMultiplier.y >= 1.0) or (cbColorMultiplierData.colorMultiplier.y <= 0.0)) then
        begin
            if cbColorMultiplierData.colorMultiplier.y >= 1.0 then
                cbColorMultiplierData.colorMultiplier.y := 1.0
            else
                cbColorMultiplierData.colorMultiplier.y := 0.0;
            gIncrement := -gIncrement;
        end;
        if ((cbColorMultiplierData.colorMultiplier.z >= 1.0) or (cbColorMultiplierData.colorMultiplier.z <= 0.0)) then
        begin
            if cbColorMultiplierData.colorMultiplier.z >= 1.0 then
                cbColorMultiplierData.colorMultiplier.z := 1.0
            else
                cbColorMultiplierData.colorMultiplier.z := 0.0;
            bIncrement := -bIncrement;
        end;
                 *)

        // copy our ConstantBuffer instance to the mapped constant buffer resource
        //        move(cbColorMultiplierData, cbColorMultiplierGPUAddress[frameIndex], sizeof(cbColorMultiplierData));
        // Windows.CopyMemory(@cbColorMultiplierGPUAddress[frameIndex], @cbColorMultiplierData, sizeof(cbColorMultiplierData));
    ...
    begin
        ...
        begin
            ...
            OutputDebugStringA(Pansichar(IntToStr(dw)));
        end;
    ...
    begin
        ...
        // dsvHandle:=TD3D12_CPU_DESCRIPTOR_HANDLE.Create();
     ...
     //   descriptorHeaps[0] := mainDescriptorHeap[frameIndex];
        commandList.SetDescriptorHeaps({length(descriptorHeaps)}1, @mainDescriptorHeap[frameIndex]);
