...
begin
    ...
    Result := m_Terrain.Initialize(Direct3D.GetDevice(), Direct3D.GetDeviceContext(), '.\data\setup.txt');
    if (Result <> S_OK) then
    begin
...
begin
    ...
    begin
        // set the Shader
        Result := ShaderManager.TerrainShader.SetShaderParametersExtern(Direct3D.GetDeviceContext(), worldMatrix,
            viewMatrix, projectionMatrix, m_Light.GetDirection(), m_Light.GetDiffuseColor());
        // Put the terrain cell buffers on the pipeline.
        Result := m_Terrain.RenderCell(Direct3D.GetDeviceContext(), i, m_Frustum, ShaderManager.TerrainShader,TextureManager);
        if (Result = S_OK) then
        begin
