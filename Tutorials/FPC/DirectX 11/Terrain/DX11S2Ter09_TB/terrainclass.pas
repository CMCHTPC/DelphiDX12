...
type
    ...
    private
        ...
        m_renderCount, m_cellsDrawn, m_cellsCulled: integer;
        m_terrainHeight, m_terrainWidth: integer;
        m_heightScale: single;
        m_terrainFilename: WideString;
        m_colorMapFilename: WideString;
        m_MaterialFilename: WideString;
        m_MaterialMapFilename: WideString;
        m_heightMap: array of THeightMapType;
        m_terrainModel: array of TModelType;
        m_TerrainCells: array of TTerrainCellClass;

        m_TextureManager: TTextureManagerClass;

        m_TextureCount: integer;
        m_Textures: array of TTextureClass;
        m_MaterialCount: integer;
        m_Materials: array of TMaterialGroupDef;
    public
        constructor Create;
        destructor Destroy; override;

        function Initialize(device: ID3D11Device; deviceContext: ID3D11DeviceContext; setupFilename: WideString): HResult;
        procedure Shutdown();
        //  function Render(deviceContext: ID3D11DeviceContext): HResult;
        //        function GetIndexCount(): int32;

        function GetCellIndexCount(cellId: integer): int32;
        function GetCellLinesIndexCount(cellId: integer): int32;
        function GetCellCount(): int32;

        function GetRenderCount(): int32;
        function GetCellsDrawn(): int32;
        function GetCellsCulled(): int32;


        function RenderCell(deviceContext: ID3D11DeviceContext; cellId: integer; Frustum: TFrustumClass;  TerrainShader: TTerrainShaderClass;TextureManager: TTextureManagerClass): HResult;
        procedure RenderCellLines(deviceContext: ID3D11DeviceContext; cellId: integer);
        procedure Frame();

        function GetHeightAtPosition(inputX, inputZ: single; var Height: single): boolean;
    private
        ...
        function LoadRawHeightMap(): HResult;
        function LoadSetupFile(filename: WideString): HResult;

        function LoadMaterialFile(filename: WideString; MapName: WideString; device: ID3D11Device; deviceContext: ID3D11DeviceContext): HResult;
        function LoadMaterialMap(filename: WideString): HResult;
        procedure ReleaseMaterials;
        ...
        procedure CalculateTangentBinormal(vertex1: TTempVertexType; vertex2: TTempVertexType; vertex3: TTempVertexType;
            var tangent: TVectorType; var binormal: TVectorType);

        function LoadTerrainCells(device: ID3D11Device): HResult;
        procedure ShutdownTerrainCells();

        function CheckHeightOfTriangle(x, z: single; var Height: single; v0, v1, v2: array of single): boolean;
    end;
...
function TTerrainClass.Initialize(device: ID3D11Device; deviceContext: ID3D11DeviceContext; setupFilename: WideString): HResult;
begin
    ...
    *)
    ...
    Result := LoadMaterialFile(m_MaterialFilename, m_MaterialMapFilename, device, deviceContext);
...
function TTerrainClass.GetCellIndexCount(cellId: integer): int32;
begin
    Result := m_TerrainCells[cellId].GetIndexCount();
end;



function TTerrainClass.GetCellLinesIndexCount(cellId: integer): int32;
begin
    Result := m_TerrainCells[cellId].GetLineBuffersIndexCount();
end;



function TTerrainClass.GetCellCount(): int32;
begin
    Result := m_cellCount;
end;
...
function TTerrainClass.GetRenderCount(): int32;
begin
    Result := m_renderCount;
end;



function TTerrainClass.GetCellsDrawn(): int32;
begin
    Result := m_cellsDrawn;
end;
...
function TTerrainClass.GetCellsCulled(): int32;
begin
    Result := m_cellsCulled;
end;



function TTerrainClass.RenderCell(deviceContext: ID3D11DeviceContext; cellId: integer; Frustum: TFrustumClass; TerrainShader: TTerrainShaderClass;TextureManager: TTextureManagerClass): HResult;
var
    maxWidth, maxHeight, maxDepth, minWidth, minHeight, minDepth: single;
    lResult: boolean;
begin
    ...
    lResult := Frustum.CheckRectangle2(maxWidth, maxHeight, maxDepth, minWidth, minHeight, minDepth);
    if (not lResult) then
    begin
    ...
    m_TerrainCells[cellId].Render(deviceContext,TerrainShader,TextureManager);

    // Add the polygons in the cell to the render count.
    m_renderCount := m_renderCount + (m_TerrainCells[cellId].GetVertexCount() div 3);
...
procedure TTerrainClass.RenderCellLines(deviceContext: ID3D11DeviceContext; cellId: integer);
begin
...
function TTerrainClass.GetHeightAtPosition(inputX, inputZ: single; var Height: single): boolean;
var
    i, cellId, index: integer;
    vertex1, vertex2, vertex3: array [0..2] of single;
    foundHeight: boolean;
    maxWidth, maxHeight, maxDepth, minWidth, minHeight, minDepth: single;
begin
    // Loop through all of the terrain cells to find out which one the inputX and inputZ would be inside.
    cellId := -1;
    for i := 0 to m_cellCount - 1 do
    begin
        ...
        if ((inputX < maxWidth) and (inputX > minWidth) and (inputZ < maxDepth) and (inputZ > minDepth)) then
        begin
            cellId := i;
            Break;
        end;
    ...
    for i := 0 to (m_TerrainCells[cellId].GetVertexCount() div 3) - 1 do
    begin
        index := i * 3;
...
function TTerrainClass.LoadRawHeightMap(): HResult;
var
    error, i, j, index: integer;
    lFileStream: TFileStream;
    imageSize, Count: uint64;
    rawImage: array of word;
begin
    Result := E_FAIL;
    ...
    SetLength(rawImage, imageSize);

    // Read in the raw image data.
    Count := lFileStream.Size;
    Count := lFileStream.Read(rawImage[0], imageSize * 2);
    if (Count <> imageSize * 2) then Exit;
    ...
    begin
        ...
        begin
            ...
            m_heightMap[index].y := rawImage[index];
        end;
    end;

    // Release the bitmap image data.
    SetLength(rawImage, 0);
    Result := S_OK;
end;
...
begin
    ...
    // Read up to the material file name.
    Read(F, input);
    while (input <> ':') do
        Read(F, input);

    // Read in the material  file name.
    ReadLn(f, lValue);
    m_MaterialFilename := trim(lValue);

    // Read up to the material map file name.
    Read(F, input);
    while (input <> ':') do
        Read(F, input);

    // Read in the material map file name.
    ReadLn(f, lValue);
    m_MaterialMapFilename := trim(lValue);
...
function TTerrainClass.LoadMaterialFile(filename: WideString; MapName: WideString; device: ID3D11Device;
...
var
    ...
    lValue: ansistring;
    lCode: integer;
    i, j, iPos, jPos: integer;
    lIdx: integer;
    lString: ansistring;
begin

    // Open the material file.  If it could not open the file then exit.
    AssignFile(F, filename);
    Reset(F);
    ...
    ReadLn(f, lValue);
    lValue := trim(lValue);
    Val(lValue, m_textureCount, lCode);
    SetLength(m_Textures, m_textureCount);
    j := 0;
    while j < m_textureCount do
    begin
        ReadLn(f, lString);
        if lString <> '' then
        begin
            i := Pos(':', lString, 1);
            lIdx := StrToInt(copy(lString, 1, i - 1));
            lString := trim(copy(lString, i + 1, Length(lString) - i));
            // load Materials
          //  Result := m_TextureManager.LoadTexture(device, DeviceContext(), lString, j);
          //  if (Result <> S_OK) then Exit;
            Inc(j);
        end;
    end;
    ...
    // Read in the value of the material count.
    ReadLn(F, lString);
    lValue := trim(lString);
    Val(lValue, m_MaterialCount, lCode);
    SetLength(m_Materials, m_MaterialCount);


    // Load each of the material group indexes in.
    i := 0;
    while i < m_materialCount do
    begin
        // 0: 0 -1 -1   0 255   0
        ReadLn(f, lString);
        if lString <> '' then
        begin
            jPos := 1;
            iPos := 1;
            while lString[jPos] <> ':' do Inc(jPos);
            lValue := trim(copy(lString, iPos, jPos - iPos));
            Val(lValue, lIdx, lCode);
            iPos := jPos + 1;

            while lString[iPos] = ' ' do Inc(iPos);
            jPos := iPos;
            while lString[jPos] <> ' ' do Inc(jPos);
            lValue := trim(copy(lString, iPos, jPos - iPos));
            Val(lValue, m_Materials[lIdx].textureIndex1, lCode);
            iPos := jPos + 1;
            ...
            while lString[jPos] <> ' ' do Inc(jPos);
            lValue := trim(copy(lString, iPos, jPos - iPos));
            Val(lValue, m_Materials[lIdx].textureIndex2, lCode);
            iPos := jPos + 1;
            ...
            while lString[jPos] <> ' ' do Inc(jPos);
            lValue := trim(copy(lString, iPos, jPos - iPos));
            Val(lValue, m_Materials[lIdx].alphaIndex, lCode);
            iPos := jPos + 1;
            ...
            while lString[jPos] <> ' ' do Inc(jPos);
            lValue := trim(copy(lString, iPos, jPos - iPos));
            Val(lValue, m_Materials[lIdx].red, lCode);
            iPos := jPos + 1;
            ...
            while lString[jPos] <> ' ' do Inc(jPos);
            lValue := trim(copy(lString, iPos, jPos - iPos));
            Val(lValue, m_Materials[lIdx].green, lCode);
            iPos := jPos;

            // while lString[iPos] = ' ' do Inc(iPos);
            jPos := Length(lString);
            lValue := trim(copy(lString, iPos, jPos - iPos + 1));
            Val(lValue, m_Materials[lIdx].blue, lCode);
    ...
    Result := LoadMaterialMap(MapName);
end;



function TTerrainClass.LoadMaterialMap(filename: WideString): HResult;
var
...
begin
    ...
    begin
        ...
        begin
            ...
            m_heightMap[index].rIndex := bitmapImage[k+2];
            m_heightMap[index].gIndex := bitmapImage[k + 1];
            m_heightMap[index].rIndex := bitmapImage[k ];
...
procedure TTerrainClass.ReleaseMaterials;
begin
...
var
    ...
    quadsCovered, incrementSize, tu2Left, tu2Right, tv2Bottom, tv2Top: single;
begin
    ...
    quadsCovered := 32.0;
    incrementSize := 1.0 / quadsCovered;
    ...
    begin
        ...
        begin
            ...
            m_terrainModel[index].rIndex := m_heightMap[index1].rIndex;
            m_terrainModel[index].bIndex := m_heightMap[index1].bIndex;
            m_terrainModel[index].gIndex := m_heightMap[index1].gIndex;
            Inc(index);
            ...
            m_terrainModel[index].tv2 := tv2Top;
            m_terrainModel[index].rIndex := m_heightMap[index2].rIndex;
            m_terrainModel[index].bIndex := m_heightMap[index2].bIndex;
            m_terrainModel[index].gIndex := m_heightMap[index2].gIndex;
            Inc(index);
            ...
            m_terrainModel[index].rIndex := m_heightMap[index3].rIndex;
            m_terrainModel[index].bIndex := m_heightMap[index3].bIndex;
            m_terrainModel[index].gIndex := m_heightMap[index3].gIndex;
            Inc(index);
            ...
            m_terrainModel[index].tu2 := tu2Right;
            m_terrainModel[index].tv2 := tv2Bottom;
            m_terrainModel[index].rIndex := m_heightMap[index4].rIndex;
            m_terrainModel[index].bIndex := m_heightMap[index4].bIndex;
            m_terrainModel[index].gIndex := m_heightMap[index4].gIndex;
            Inc(index);

            // Increment the second tu texture coords.
            tu2Left := tu2Left + incrementSize;
            tu2Right := tu2Right + incrementSize;

            // Reset the second tu texture coordinate increments.
            if (tu2Right > 1.0) then
            begin
                tu2Left := 0.0;
                tu2Right := incrementSize;
            end;
        ...
        tv2Top := tv2Top + incrementSize;
        tv2Bottom := tv2Bottom + incrementSize;

        // Reset the second tu texture coordinate increments.
        if (tv2Bottom > 1.0) then
        begin
            tv2Top := 0.0;
            tv2Bottom := incrementSize;
        end;
...
procedure TTerrainClass.CalculateTangentBinormal(vertex1: TTempVertexType; vertex2: TTempVertexType; vertex3: TTempVertexType;
    var tangent: TVectorType; var binormal: TVectorType);
var
...
function TTerrainClass.LoadTerrainCells(device: ID3D11Device): HResult;
var
    cellHeight, cellWidth, cellRowCount, i, j, index: integer;
begin
    ...
    cellRowCount := (m_terrainWidth - 1) div (cellWidth - 1);
    m_cellCount := cellRowCount * cellRowCount;

    // Create the terrain cell array.
    SetLength(m_TerrainCells, m_cellCount);



    // Loop through and initialize all the terrain cells.
    for j := 0 to cellRowCount - 1 do
    begin
        for i := 0 to cellRowCount - 1 do
        begin
            index := (cellRowCount * j) + i;
            m_TerrainCells[index] := TTerrainCellClass.Create;
            Result := m_TerrainCells[index].Initialize(device, @m_terrainModel[0], i, j, cellHeight, cellWidth, m_terrainWidth,m_Materials);
            if (Result <> S_OK) then Exit;
        end;
    end;
...
procedure TTerrainClass.ShutdownTerrainCells();
var
    i: integer;
begin
    ...
    begin
        ...
        m_TerrainCells[i].Free;
        m_TerrainCells[i] := nil;
    end;
    SetLength(m_TerrainCells, 0);
end;



function TTerrainClass.CheckHeightOfTriangle(x, z: single; var Height: single; v0, v1, v2: array of single): boolean;
var
    startVector, directionVector, edge1, edge2, normal: array [0..2] of single;
    Q, e1, e2, e3, edgeNormal, temp: array [0..2] of single;
    magnitude, D, denominator, numerator, t, determinant: single;
begin
    ...
    numerator := -1.0 * (((normal[0] * startVector[0]) + (normal[1] * startVector[1]) + (normal[2] * startVector[2])) + D);
