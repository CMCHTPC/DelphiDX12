...
type
    ...
    public
        ...
        function SetIntersection(intersection: boolean; deviceContext: ID3D11DeviceContext): HResult;
    end;
...
begin
    ...
    Result := InitializeSentence(m_sentence1, 20, device);
    ...
    // Initialize the first sentence.
    Result := InitializeSentence(m_sentence2, 20, device);
    if (Result <> S_OK) then  Exit;
...
function TTextClass.SetIntersection(intersection: boolean; deviceContext: ID3D11DeviceContext): HResult;
begin
    if (intersection) then
        Result := UpdateSentence(m_sentence1, 'Intersection: Yes', 20, 20, 0.0, 1.0, 0.0, deviceContext)

    else
        Result := UpdateSentence(m_sentence1, 'Intersection: No', 20, 20, 0.0, 1.0, 0.0, deviceContext);
