...
begin
    ...
    Result := m_SkyPlane.Initialize(Direct3D.GetDevice(), Direct3D.GetDeviceContext(), '.\data\cloud001.dds', '.\data\cloud002.dds');
    if (Result <> S_OK) then
    begin
...
begin
    ...
    Result := ShaderManager.RenderSkyPlaneShader(Direct3D.GetDeviceContext(), m_SkyPlane.GetIndexCount(), worldMatrix,
        viewMatrix, projectionMatrix, m_SkyPlane.GetCloudTexture1(), m_SkyPlane.GetCloudTexture2(), m_SkyPlane.GetTranslation(0),
        m_SkyPlane.GetTranslation(1), m_SkyPlane.GetTranslation(2), m_SkyPlane.GetTranslation(3), m_SkyPlane.GetBrightness());
    if (Result <> S_OK) then
        Exit;
