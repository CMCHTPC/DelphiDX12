...
type
    ...
    private
        ...
        m_reflectionViewMatrix: TXMMATRIX;
    public
        constructor Create;
        ...
        procedure RenderReflection(Height: single);
        procedure GetReflectionViewMatrix(out viewMatrix: TXMMATRIX);
    end;
...
procedure TCameraClass.RenderReflection(Height: single);
var
    up, position, lookAt: TXMFLOAT3;
    upVector, positionVector, lookAtVector: TXMVECTOR;
    yaw, pitch, roll: single;
...
begin
    ...
    // Setup the position of the camera in the world.
    position.x := m_positionX;
    position.y := -m_positionY + (Height * 2.0);
    position.z := m_positionZ;
...
procedure TCameraClass.GetReflectionViewMatrix(out viewMatrix: TXMMATRIX);
begin
