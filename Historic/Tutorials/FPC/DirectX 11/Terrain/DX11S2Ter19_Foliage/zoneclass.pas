...
begin
    ...
    m_Position.SetPosition(0.0, 1.5, -4.0);
    m_Position.SetRotation(15.0, 0.0, 0.0);
    ...
    Result := m_GroundModel.Initialize(Direct3D.GetDevice(), Direct3D.GetDeviceContext(), '.\data\plane01.txt', '.\data\rock015.dds');
    if (Result <> S_OK) then
    begin
    ...
    m_Foliage := TFoliageClass.Create;

    // Initialize the foliage object.
    Result := m_Foliage.Initialize(Direct3D.GetDevice(), Direct3D.GetDeviceContext(), '.\data\grass.dds', 500);
    if (Result <> S_OK) then
    begin
        MessageBoxW(hwnd, 'Could not initialize the foliage object.', 'Error', MB_OK);
        Exit;
    end;
...
begin
    ...
    Result := m_Foliage.Frame(cameraPosition, Direct3D.GetDeviceContext());
    if (Result <> S_OK) then
        Exit;
...
begin
	...
	m_GroundModel.Render(Direct3D.GetDeviceContext());
	ShaderManager.RenderTextureShader(Direct3D.GetDeviceContext(), m_GroundModel.GetIndexCount(), worldMatrix, viewMatrix, projectionMatrix, m_GroundModel.GetTexture());

	// Turn on the alpha-to-coverage blending.
	Direct3D.EnableAlphaToCoverageBlending();

	// Render the foliage.
	m_Foliage.Render(Direct3D.GetDeviceContext());
	ShaderManager.RenderFoliageShader(Direct3D.GetDeviceContext(), m_Foliage.GetVertexCount(), m_Foliage.GetInstanceCount(), viewMatrix, projectionMatrix, m_Foliage.GetTexture());
