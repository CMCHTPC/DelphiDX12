...
begin
    ...
    //create D2D factory
    hr := D2D1CreateFactory(D2D1_FACTORY_TYPE_SINGLE_THREADED  , @IID_ID2D1Factory, @pFactoryOptions, m_pD2DFactory);
    if (SUCCEEDED(hr)) then
    begin
        //create DWrite factory
        hr := DWriteCreateFactory(DWRITE_FACTORY_TYPE_ISOLATED, //DWRITE_FACTORY_TYPE_SHARED
            @IID_IDWriteFactory, m_pDWriteFactory);
    end;
...
begin
    ...
    begin
        ...
        // flag.
        ap := RenderTargetProperties();
        ap2 := HwndRenderTargetProperties(m_hwnd, size, D2D1_PRESENT_OPTIONS_IMMEDIATELY);
        ...
        begin
            ...
            hr := m_pRT.CreateSolidColorBrush(ColorF(Black),m_pBlackBrush); // calling the helper function
            {$ELSE}
...
begin
    ...
    begin
        ...
        begin
            ...
            offset := Point2F(-textMetrics.Width / 2.0 - m_overhangOffset.x, -textMetrics.Height / 2.0 - m_overhangOffset.y);
            ...
            roundedOffset := Point2F((offset.x * dpiX / 96.0 + 0.5) * 96.0 / dpiX, (offset.y * dpiY / 96.0 + 0.5) * 96.0 / dpiY);
            destinationRect := RectF(roundedOffset.x, roundedOffset.y, roundedOffset.x + opacityRTSize.Width,
        ...
        begin
            ...
            m_pRT.DrawTextLayout(
                Point2F(-textMetrics.Width / 2.0, -textMetrics.Height / 2.0),
                m_pTextLayout,
                m_pBlackBrush,
...
begin
    ...
    begin
        ...
        // padding.
        padding := SizeF(96.0 / dpiX, 96.0 / dpiY);
        m_overhangOffset := Point2F(trunc(overhangMetrics.left + padding.Width), trunc(overhangMetrics.top + padding.Height));
        // The true width of the text is the max width + the overhang
        // metrics + padding in each direction.
        maskSize := SizeF(overhangMetrics.right + padding.Width + m_overhangOffset.x + m_pTextLayout.GetMaxWidth(),
            overhangMetrics.bottom + padding.Height + m_overhangOffset.y + m_pTextLayout.GetMaxHeight());
        // Round up to the nearest pixel
        maskPixelSize := SizeU(trunc(maskSize.Width * dpiX / 96.0), trunc(maskSize.Height * dpiY / 96.0));
        ...
        alphaOnlyFormat := PixelFormat(DXGI_FORMAT_A8_UNORM, D2D1_ALPHA_MODE_PREMULTIPLIED);
        ...
        begin
            ...
            m_pOpacityRT.Clear(ColorF(Black, 0.0));
...
begin
    ...
    pTransform := Matrix3x2F_Rotation(rotation, Point2F(0, 0)) * Matrix3x2F_Scale(scaleMultiplier,
        scaleMultiplier, Point2F(0, 0)) * Matrix3x2F_Translation(translationOffset + size.Width / 2.0,
