...
uses
    ...
    DX12.D2D1,
    DX12.D2D1_1,
    DX12.WinCodec, Win32.COMBaseAPI;
...
begin
    ...
    begin
        // Create D2D factory
        hr := D2D1CreateFactory(D2D1_FACTORY_TYPE_SINGLE_THREADED, @IID_ID2D1Factory1, nil, m_pD2DFactory);
    end;

    if (SUCCEEDED(hr)) then
    begin
        // Create WIC factory
        hr := CoCreateInstance(@CLSID_WICImagingFactory, nil, CLSCTX_INPROC_SERVER, @IID_IWICImagingFactory, m_pIWICFactory);
    end;
end;
...
var
    rcClient: TRECT;
    lRenderTargetProperties: TD2D1_RENDER_TARGET_PROPERTIES;
...
begin
    ...
    begin
        ...
        begin
            ...
            lRenderTargetProperties := RenderTargetProperties();

            // Set the DPI to be the default system DPI to allow direct mapping
            // between image pixels and desktop pixels in different system DPI
            // settings
            lRenderTargetProperties.dpiX := DEFAULT_DPI;
            lRenderTargetProperties.dpiY := DEFAULT_DPI;

            hwndRenderTragetproperties :=
                HwndRenderTargetProperties(m_hWnd, SizeU(RectWidth(rcClient), RectHeight(rcClient)));

            Result := m_pD2DFactory.CreateHwndRenderTarget(lRenderTargetProperties, hwndRenderTragetproperties, m_pHwndRT);
        end
        else
        begin
    ...
    begin
        ...
        SafeRelease(m_pFrameComposeRT);
        fSize:= SizeF(m_cxGifImage, m_cyGifImage);
        Result := m_pHwndRT.CreateCompatibleRenderTarget(@fSize, nil, nil,
...
begin
    ...
    begin
        ...
        begin
            ...
            m_backgroundColor := ColorF(0, 0.0);
        end;
    end;
