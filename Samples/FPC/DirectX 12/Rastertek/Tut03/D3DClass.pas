...
function CreateEventExW(lpEventAttributes: LPSECURITY_ATTRIBUTES; lpName: LPCWSTR; dwFlags: DWORD; dwDesiredAccess: DWORD): HANDLE; stdcall; external 'Kernel32.dll';
...
begin
    ...
    commandQueueDesc.CommandListType := D3D12_COMMAND_LIST_TYPE_DIRECT;
    commandQueueDesc.Priority := Ord(D3D12_COMMAND_QUEUE_PRIORITY_NORMAL);
    ...
    renderTargetViewHeapDesc.HeapType := D3D12_DESCRIPTOR_HEAP_TYPE_RTV;
    ...
    m_renderTargetViewHeap.GetCPUDescriptorHandleForHeapStart(@renderTargetViewHandle);
...
begin
    ...
    x := sizeof(TD3D12_RESOURCE_BARRIER);
    barrier.Flags := D3D12_RESOURCE_BARRIER_FLAG_NONE;
    barrier.Transition.pResource := PID3D12Resource(m_backBufferRenderTarget[m_bufferIndex]);
    barrier.Transition.StateBefore := D3D12_RESOURCE_STATE_PRESENT;
    barrier.Transition.StateAfter := D3D12_RESOURCE_STATE_RENDER_TARGET;
    barrier.Transition.Subresource := D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
    barrier.BarrierType := D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
    ...
    color[2] := 0.5;
    color[3] := 1.0;
    m_commandList.ClearRenderTargetView(renderTargetViewHandle, TFloatArray4(color), 0, nil);
