...
begin
    ...
    srvHeapDesc.HeapType := D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV;
...
var
    ...
    resource_desc: TD3D12_RESOURCE_DESC;
    HEAP_PROPERTIES: TD3D12_HEAP_PROPERTIES;
begin
    ...
    rootSignatureDesc.Init_1_1(Length(rootParameters), @rootParameters[0], 1, @sampler,
...
{$endif}
    ...
    hr := D3DCompileFromFile(pwidechar(GetAssetFullPath('.\data\shaders.hlsl')), nil, nil, 'VSMain', 'vs_5_0', compileFlags, 0, vertexShader, @error);
    hr := D3DCompileFromFile(pwidechar(GetAssetFullPath('.\data\shaders.hlsl')), nil, nil, 'PSMain', 'ps_5_0', compileFlags, 0, pixelShader, @error);
    ...
    psoDesc.VS.Create(vertexShader);
    psoDesc.PS.Create(pixelShader);
    psoDesc.RasterizerState := TD3D12_RASTERIZER_DESC.Create(D3D12_DEFAULT);
    psoDesc.BlendState := TD3D12_BLEND_DESC.Create(D3D12_DEFAULT);
    ...
    ThrowIfFailed(m_device.CreateCommittedResource(TD3D12_HEAP_PROPERTIES.Create(D3D12_HEAP_TYPE_UPLOAD),
        D3D12_HEAP_FLAG_NONE, TD3D12_RESOURCE_DESC.CreateBuffer(vertexBufferSize), D3D12_RESOURCE_STATE_GENERIC_READ,
    ...
    ThrowIfFailed(m_device.CreateCommittedResource(TD3D12_HEAP_PROPERTIES.Create(D3D12_HEAP_TYPE_DEFAULT),
    ...
    ThrowIfFailed(m_device.CreateCommittedResource(TD3D12_HEAP_PROPERTIES.Create(D3D12_HEAP_TYPE_UPLOAD),
        D3D12_HEAP_FLAG_NONE, TD3D12_RESOURCE_DESC.CreateBuffer(uploadBufferSize), D3D12_RESOURCE_STATE_GENERIC_READ,
    ...
    UpdateSubresources_Heap(m_commandList, m_texture, textureUploadHeap, 0, 0, 1, @textureData);
    m_commandList.ResourceBarrier(1, TD3D12_RESOURCE_BARRIER.CreateTransition(m_texture, D3D12_RESOURCE_STATE_COPY_DEST,
...
const
    clearColor: TFloatArray4 = (0.0, 0.2, 0.4, 1.0);
var
    rtvHandle: TD3D12_CPU_DESCRIPTOR_HANDLE;
...
begin
    ...
    m_rtvHeap.GetCPUDescriptorHandleForHeapStart(@rtvHandle);
    rtvHandle := TD3D12_CPU_DESCRIPTOR_HANDLE.Create(@rtvHandle, m_frameIndex, m_rtvDescriptorSize);
...
begin
    ...
    m_viewport.Init(0.0, 0.0, Width, Height);
    m_scissorRect.init(0, 0, Width, Height);
