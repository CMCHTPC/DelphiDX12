...
type
    ...
    private
        ...
        m_Model: TModelClass;
        m_CubeModel,m_GroundModel,m_SphereModel: TModelClass;
...
begin
    ...
    Result := m_Model.Initialize(m_Direct3D.GetDevice(), m_Direct3D.GetDeviceContext(), '.\data\cube.txt', '.\data\seafloor.dds');
    if (Result <> S_OK) then
    begin
    ...
    Result := m_FloorModel.Initialize(m_Direct3D.GetDevice(), m_Direct3D.GetDeviceContext(), '.\data\floor.txt', '.\data\blue01.dds');
    if (Result <> S_OK) then
    begin
        MessageBoxW(hwnd, 'Could not initialize the model object.', 'Error', MB_OK);
        Exit;
