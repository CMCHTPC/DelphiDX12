...
begin
    ...
    {$IFDEF TextureSelf}
    {$IFDEF DDS}
    Result := CreateDDSTextureFromFile(device, '.\data\seafloor.dds', ID3D11Resource(m_texture), m_textureView, maxsize, alphaMode);
    {$ELSE}
    Result := CreateTGATextureFromFile(device, deviceContext,filename, ID3D11Resource(m_texture), m_textureView);
    {$ENDIF}
    {$ELSE}
    // Load the texture in.
    filename := '.\data\seafloor.dds';
