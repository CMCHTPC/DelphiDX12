...
type
    ...
    TTranslateBufferType = record
        translation: single;
        padding: TXMFLOAT3;
    end;
    { TTranslateShaderClass }

    TTranslateShaderClass = class(TObject)
    private
        ...
        m_translateBuffer: ID3D11Buffer;
    private
        ...
        function SetShaderParameters(deviceContext: ID3D11DeviceContext; worldMatrix, viewMatrix, projectionMatrix: TXMMATRIX;
            texture: ID3D11ShaderResourceView; Translation: single): HResult;
...
function TTranslateShaderClass.InitializeShader(device: ID3D11Device; hwnd: HWND; vsFilename, psFilename: WideString): HResult;
var
    ...
    translateBufferDesc: TD3D11_BUFFER_DESC;
begin
    // Initialize the pointers this function will use to null.


    // Compile the vertex shader code.
    Result := D3DCompileFromFile(pwidechar(vsFilename), nil, nil, 'TranslateVertexShader', 'vs_5_0',
    ...
    Result := D3DCompileFromFile(pwidechar(psFilename), nil, nil, 'TranslatePixelShader', 'ps_5_0',
    ...
    translateBufferDesc.ByteWidth := sizeof(TTranslateBufferType);
    translateBufferDesc.BindFlags := ord(D3D11_BIND_CONSTANT_BUFFER);
    translateBufferDesc.CPUAccessFlags := ord(D3D11_CPU_ACCESS_WRITE);
    translateBufferDesc.MiscFlags := 0;
    translateBufferDesc.StructureByteStride := 0;

    // Create the constant buffer pointer so we can access the pixel shader constant buffer from within this class.
    Result := device.CreateBuffer(translateBufferDesc, nil, m_translateBuffer);

end;



procedure TTranslateShaderClass.ShutdownShader();
begin
    m_translateBuffer := nil;
...
procedure TTranslateShaderClass.OutputShaderErrorMessage(errorMessage: ID3D10Blob; hwnd: HWND; shaderFilename: WideString);
var
...
function TTranslateShaderClass.SetShaderParameters(deviceContext: ID3D11DeviceContext; worldMatrix, viewMatrix, projectionMatrix: TXMMATRIX;
    texture: ID3D11ShaderResourceView; Translation: single): HResult;
var
    mappedResource: TD3D11_MAPPED_SUBRESOURCE;
    dataPtr: ^TMatrixBufferType;
    bufferNumber: UINT;
    dataPtr2: ^TTranslateBufferType;
begin
...
procedure TTranslateShaderClass.RenderShader(deviceContext: ID3D11DeviceContext; indexCount: integer);
begin
...
constructor TTranslateShaderClass.Create;
begin
...
destructor TTranslateShaderClass.Destroy;
begin
    inherited Destroy;
end;
...
function TTranslateShaderClass.Initialize(device: ID3D11Device; hwnd: HWND): Hresult;
begin
    // Initialize the vertex and pixel shaders.
    Result := InitializeShader(device, hwnd, '.\data\translate.vs', '.\data\translate.ps');
end;



procedure TTranslateShaderClass.Shutdown();
begin
...
function TTranslateShaderClass.Render(deviceContext: ID3D11DeviceContext; indexCount: integer;
    worldMatrix, viewMatrix, projectionMatrix: TXMMATRIX; texture: ID3D11ShaderResourceView; Translation: single): HResult;
begin
