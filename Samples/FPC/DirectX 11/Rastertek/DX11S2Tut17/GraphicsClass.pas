...
type
    ...
    private
        ...
        m_Model: TModelClass;
        m_MultiTextureShader: TMultiTextureShaderClass;
        FRotation: single;
    private
...
begin
    ...
    m_MultiTextureShader.Render(m_Direct3D.GetDeviceContext(),
...
begin
    ...
    Result := m_Model.Initialize(m_Direct3D.GetDevice(), m_Direct3D.GetDeviceContext(), '.\data\square.txt', '.\data\stone01.dds', '.\data\dirt01.dds');
    if (Result <> S_OK) then
    begin
    ...
    m_MultiTextureShader := TMultiTextureShaderClass.Create;

    // Initialize the multitexture shader object.
    Result := m_MultiTextureShader.Initialize(m_Direct3D.GetDevice(), hwnd);
    if (Result <> S_OK) then
    begin
        MessageBoxW(hwnd, 'Could not initialize the multitexture object.', 'Error', MB_OK);
        Exit;
    end;
...
begin
    ...
    if (m_MultiTextureShader <> nil) then
    begin
        m_MultiTextureShader.Shutdown();
        m_MultiTextureShader.Free;
        m_MultiTextureShader := nil;
    end;
