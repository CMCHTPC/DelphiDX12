...
type
    ...
    { TLightMapShaderClass }

    TLightMapShaderClass = class(TObject)
    private
...
function TLightMapShaderClass.InitializeShader(device: ID3D11Device; hwnd: HWND; vsFilename, psFilename: WideString): HResult;
var
...
begin
    ...
    Result := D3DCompileFromFile(pwidechar(vsFilename), nil, nil, 'LightMapVertexShader', 'vs_5_0',
    ...
    Result := D3DCompileFromFile(pwidechar(psFilename), nil, nil, 'LightMapPixelShader', 'ps_5_0', D3D10_SHADER_ENABLE_STRICTNESS,
...
procedure TLightMapShaderClass.ShutdownShader();
begin
...
procedure TLightMapShaderClass.OutputShaderErrorMessage(errorMessage: ID3D10Blob; hwnd: HWND; shaderFilename: WideString);
var
...
function TLightMapShaderClass.SetShaderParameters(deviceContext: ID3D11DeviceContext; worldMatrix, viewMatrix, projectionMatrix: TXMMATRIX;
    textureArray: PID3D11ShaderResourceView): HResult;
var
...
procedure TLightMapShaderClass.RenderShader(deviceContext: ID3D11DeviceContext; indexCount: integer);
begin
...
constructor TLightMapShaderClass.Create;
begin
...
destructor TLightMapShaderClass.Destroy;
begin
    inherited Destroy;
end;
...
function TLightMapShaderClass.Initialize(device: ID3D11Device; hwnd: HWND): Hresult;
begin
    // Initialize the vertex and pixel shaders.
    Result := InitializeShader(device, hwnd, '.\data\lightmap.vs', '.\data\lightmap.ps');
end;



procedure TLightMapShaderClass.Shutdown();
begin
...
function TLightMapShaderClass.Render(deviceContext: ID3D11DeviceContext; indexCount: integer;
