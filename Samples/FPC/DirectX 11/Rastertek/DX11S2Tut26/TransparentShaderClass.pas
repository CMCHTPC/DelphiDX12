...
type
    ...
    TTransparentBufferType = record
        blendAmount: single;
        padding: TXMFLOAT3;
    end;

    { TTransparentShaderClass }

    TTransparentShaderClass = class(TObject)
    private
        ...
        m_transparentBuffer: ID3D11Buffer;
    private
        ...
        function SetShaderParameters(deviceContext: ID3D11DeviceContext; worldMatrix, viewMatrix, projectionMatrix: TXMMATRIX;
            texture: ID3D11ShaderResourceView; blend: single): HResult;
...
function TTransparentShaderClass.InitializeShader(device: ID3D11Device; hwnd: HWND; vsFilename, psFilename: WideString): HResult;
var
    ...
    transparentBufferDesc: TD3D11_BUFFER_DESC;
begin
    // Initialize the pointers this function will use to null.


    // Compile the vertex shader code.
    Result := D3DCompileFromFile(pwidechar(vsFilename), nil, nil, 'TransparentVertexShader', 'vs_5_0',
    ...
    Result := D3DCompileFromFile(pwidechar(psFilename), nil, nil, 'TransparentPixelShader', 'ps_5_0',
    ...
    transparentBufferDesc.ByteWidth := sizeof(TTransparentBufferType);
    transparentBufferDesc.BindFlags := Ord(D3D11_BIND_CONSTANT_BUFFER);
    transparentBufferDesc.CPUAccessFlags := Ord(D3D11_CPU_ACCESS_WRITE);
    transparentBufferDesc.MiscFlags := 0;
    transparentBufferDesc.StructureByteStride := 0;

    // Create the constant buffer pointer so we can access the pixel shader constant buffer from within this class.
    Result := device.CreateBuffer(transparentBufferDesc, nil, m_transparentBuffer);

end;



procedure TTransparentShaderClass.ShutdownShader();
begin
    m_transparentBuffer := nil;
...
procedure TTransparentShaderClass.OutputShaderErrorMessage(errorMessage: ID3D10Blob; hwnd: HWND; shaderFilename: WideString);
var
...
function TTransparentShaderClass.SetShaderParameters(deviceContext: ID3D11DeviceContext; worldMatrix, viewMatrix, projectionMatrix: TXMMATRIX;
    texture: ID3D11ShaderResourceView; blend: single): HResult;
var
    mappedResource: TD3D11_MAPPED_SUBRESOURCE;
    dataPtr: ^TMatrixBufferType;
    bufferNumber: UINT;
    dataPtr2: ^TTransparentBufferType;
begin
...
procedure TTransparentShaderClass.RenderShader(deviceContext: ID3D11DeviceContext; indexCount: integer);
begin
...
constructor TTransparentShaderClass.Create;
begin
...
destructor TTransparentShaderClass.Destroy;
begin
    inherited Destroy;
end;
...
function TTransparentShaderClass.Initialize(device: ID3D11Device; hwnd: HWND): Hresult;
begin
    // Initialize the vertex and pixel shaders.
    Result := InitializeShader(device, hwnd, '.\data\transparent.vs', '.\data\transparent.ps');
end;



procedure TTransparentShaderClass.Shutdown();
begin
...
function TTransparentShaderClass.Render(deviceContext: ID3D11DeviceContext; indexCount: integer;
    worldMatrix, viewMatrix, projectionMatrix: TXMMATRIX; texture: ID3D11ShaderResourceView; blend: single): HResult;
begin
