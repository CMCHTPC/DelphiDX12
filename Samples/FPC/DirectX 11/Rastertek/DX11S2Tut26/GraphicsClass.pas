...
begin
    ...
    Result := m_Model1.Initialize(m_Direct3D.GetDevice(), m_Direct3D.GetDeviceContext(), '.\data\square.txt', '.\data\dirt01.dds');
    if (Result <> S_OK) then
    begin
    ...
    Result := m_Model2.Initialize(m_Direct3D.GetDevice(), m_Direct3D.GetDeviceContext(), '.\data\square.txt', '.\data\stone01.dds');
    if (Result <> S_OK) then
    begin
        MessageBoxW(hwnd, 'Could not initialize the model object.', 'Error', MB_OK);
        Exit;
