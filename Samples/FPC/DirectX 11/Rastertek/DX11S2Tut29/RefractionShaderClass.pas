...
type
    ...
    TClipPlaneBufferType = record
        clipPlane: TXMFLOAT4;
    end;

    { TRefractionShaderClass }

    TRefractionShaderClass = class(TObject)
    private
        ...
        m_clipPlaneBuffer: ID3D11Buffer;
    ...
    private
        ...
        function SetShaderParameters(deviceContext: ID3D11DeviceContext; worldMatrix, viewMatrix, projectionMatrix: TXMMATRIX;
            texture: ID3D11ShaderResourceView; lightDirection: TXMFLOAT3; ambientColor, diffuseColor: TXMFLOAT4;
            clipPlane: TXMFLOAT4): HResult;
...
function TRefractionShaderClass.InitializeShader(device: ID3D11Device; hwnd: HWND; vsFilename, psFilename: WideString): HResult;
var
    ...
    clipPlaneBufferDesc: TD3D11_BUFFER_DESC;
...
begin
    ...
    Result := D3DCompileFromFile(pwidechar(vsFilename), nil, nil, 'RefractionVertexShader', 'vs_5_0',
    ...
    Result := D3DCompileFromFile(pwidechar(psFilename), nil, nil, 'RefractionPixelShader', 'ps_5_0',
    ...
    clipPlaneBufferDesc.ByteWidth := sizeof(TClipPlaneBufferType);
    clipPlaneBufferDesc.BindFlags := Ord(D3D11_BIND_CONSTANT_BUFFER);
    clipPlaneBufferDesc.CPUAccessFlags := Ord(D3D11_CPU_ACCESS_WRITE);
    clipPlaneBufferDesc.MiscFlags := 0;
    clipPlaneBufferDesc.StructureByteStride := 0;

    // Create the constant buffer pointer so we can access the vertex shader constant buffer from within this class.
    Result := device.CreateBuffer(clipPlaneBufferDesc, nil, m_clipPlaneBuffer);

end;



procedure TRefractionShaderClass.ShutdownShader();
begin
    m_clipPlaneBuffer := nil;
    m_lightBuffer := nil;
...
procedure TRefractionShaderClass.OutputShaderErrorMessage(errorMessage: ID3D10Blob; hwnd: HWND; shaderFilename: WideString);
var
...
function TRefractionShaderClass.SetShaderParameters(deviceContext: ID3D11DeviceContext; worldMatrix, viewMatrix, projectionMatrix: TXMMATRIX;
    texture: ID3D11ShaderResourceView; lightDirection: TXMFLOAT3; ambientColor, diffuseColor: TXMFLOAT4; clipPlane: TXMFLOAT4): HResult;
var
    mappedResource: TD3D11_MAPPED_SUBRESOURCE;
    dataPtr: ^TMatrixBufferType;
    bufferNumber: UINT;
    dataPtr2: ^TLightBufferType;
    dataPtr3: ^TClipPlaneBufferType;
begin
...
procedure TRefractionShaderClass.RenderShader(deviceContext: ID3D11DeviceContext; indexCount: integer);
begin
...
constructor TRefractionShaderClass.Create;
begin
...
destructor TRefractionShaderClass.Destroy;
begin
    inherited Destroy;
end;
...
function TRefractionShaderClass.Initialize(device: ID3D11Device; hwnd: HWND): Hresult;
begin
    // Initialize the vertex and pixel shaders.
    Result := InitializeShader(device, hwnd, '.\data\refraction.vs', '.\data\refraction.ps');
end;



procedure TRefractionShaderClass.Shutdown();
begin
