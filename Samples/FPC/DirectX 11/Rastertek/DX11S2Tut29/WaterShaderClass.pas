...
type
    ...
    TWaterBufferType = record
        waterTranslation: single;
        reflectRefractScale: single;
        padding: TXMFLOAT2;
    ...
    TWaterShaderClass = class(TObject)
    private
    ...
    private
        ...
        function SetShaderParameters(deviceContext: ID3D11DeviceContext;
            worldMatrix, viewMatrix, projectionMatrix, reflectionMatrix: TXMMATRIX;
            reflectionTexture,refractionTexture: ID3D11ShaderResourceView; normalTexture: ID3D11ShaderResourceView;
            waterTranslation, reflectRefractScale: single): HResult;
    ...
    public
        ...
        function Render(deviceContext: ID3D11DeviceContext; indexCount: integer;
            worldMatrix, viewMatrix, projectionMatrix, reflectionMatrix: TXMMATRIX; reflectionTexture, refractionTexture: ID3D11ShaderResourceView;
            normalTexture: ID3D11ShaderResourceView; waterTranslation, reflectRefractScale: single): HResult;
...
function TWaterShaderClass.InitializeShader(device: ID3D11Device; hwnd: HWND; vsFilename, psFilename: WideString): HResult;
var
    ...
    waterBufferDesc: TD3D11_BUFFER_DESC;
begin
    // Initialize the pointers this function will use to null.


    // Compile the vertex shader code.
    Result := D3DCompileFromFile(pwidechar(vsFilename), nil, nil, 'WaterVertexShader', 'vs_5_0', D3D10_SHADER_ENABLE_STRICTNESS,
    ...
    Result := D3DCompileFromFile(pwidechar(psFilename), nil, nil, 'WaterPixelShader', 'ps_5_0', D3D10_SHADER_ENABLE_STRICTNESS, 0,
    ...
    waterBufferDesc.ByteWidth := sizeof(TWaterBufferType);
    waterBufferDesc.BindFlags := Ord(D3D11_BIND_CONSTANT_BUFFER);
    waterBufferDesc.CPUAccessFlags := Ord(D3D11_CPU_ACCESS_WRITE);
    waterBufferDesc.MiscFlags := 0;
    waterBufferDesc.StructureByteStride := 0;

    // Create the constant buffer pointer so we can access the pixel shader constant buffer from within this class.
    Result := device.CreateBuffer(waterBufferDesc, nil, m_waterBuffer);

end;



procedure TWaterShaderClass.ShutdownShader();
begin
    m_waterBuffer := nil;
...
procedure TWaterShaderClass.OutputShaderErrorMessage(errorMessage: ID3D10Blob; hwnd: HWND; shaderFilename: WideString);
var
...
function TWaterShaderClass.SetShaderParameters(deviceContext: ID3D11DeviceContext;
    worldMatrix, viewMatrix, projectionMatrix, reflectionMatrix: TXMMATRIX; reflectionTexture, refractionTexture: ID3D11ShaderResourceView;
    normalTexture: ID3D11ShaderResourceView; waterTranslation, reflectRefractScale: single): HResult;
var
...
begin
    ...
    dataPtr3.padding := TXMFLOAT2.Create(0.0, 0.0);
...
procedure TWaterShaderClass.RenderShader(deviceContext: ID3D11DeviceContext; indexCount: integer);
begin
...
constructor TWaterShaderClass.Create;
begin
...
destructor TWaterShaderClass.Destroy;
begin
    inherited Destroy;
end;
...
function TWaterShaderClass.Initialize(device: ID3D11Device; hwnd: HWND): Hresult;
begin
    // Initialize the vertex and pixel shaders.
    Result := InitializeShader(device, hwnd, '.\data\water.vs', '.\data\water.ps');
end;



procedure TWaterShaderClass.Shutdown();
begin
...
function TWaterShaderClass.Render(deviceContext: ID3D11DeviceContext; indexCount: integer;
    worldMatrix, viewMatrix, projectionMatrix, reflectionMatrix: TXMMATRIX; reflectionTexture,refractionTexture: ID3D11ShaderResourceView;
    normalTexture: ID3D11ShaderResourceView; waterTranslation, reflectRefractScale: single): HResult;
begin
    // Set the shader parameters that it will use for rendering.
    Result := SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, reflectionMatrix,
        reflectionTexture, refractionTexture, normalTexture, waterTranslation, reflectRefractScale);
