...
type
    ...
    private
        ...
        m_Model: TModelClass;
        m_AlphaMapShader: TAlphaMapShaderClass;
        FRotation: single;
    private
...
begin
    ...
    m_AlphaMapShader.Render(m_Direct3D.GetDeviceContext(),
...
begin
    ...
    Result := m_Model.Initialize(m_Direct3D.GetDevice(), m_Direct3D.GetDeviceContext(), '.\data\square.txt', '.\data\stone01.dds', '.\data\dirt01.dds','.\data\alpha01.dds');
    if (Result <> S_OK) then
    begin
    ...
    m_AlphaMapShader := TAlphaMapShaderClass.Create;

    // Initialize the alpha map shader object.
    Result := m_AlphaMapShader.Initialize(m_Direct3D.GetDevice(), hwnd);
    if (Result <> S_OK) then
    begin
        MessageBoxW(hwnd, 'Could not initialize the alpha map shader object.', 'Error', MB_OK);
        Exit;
    end;
...
begin
    ...
    if (m_AlphaMapShader <> nil) then
    begin
        m_AlphaMapShader.Shutdown();
        m_AlphaMapShader.Free;
        m_AlphaMapShader := nil;
    end;
