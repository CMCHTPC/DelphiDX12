...
type
    ...
    TLightBufferType = record
        ...
        specularPower: single;  // Added extra padding so structure is a multiple of 16 for CreateBuffer function requirements.
    ...
    TLightColorBufferType = record
        diffuseColor: array [0..NUM_LIGHTS - 1] of TXMFLOAT4;
    end;

    TLightPositionBufferType = record
        lightPosition: array [0..NUM_LIGHTS - 1] of TXMFLOAT4;
    end;
    ...
    private
        ...
        m_lightPositionBuffer: ID3D11Buffer;
    private
        ...
        function SetShaderParameters(deviceContext: ID3D11DeviceContext; worldMatrix, viewMatrix, projectionMatrix: TXMMATRIX;
            texture: ID3D11ShaderResourceView; diffuseColor,lightPosition: array of TXMFLOAT4): HResult;
    ...
    public
        ...
        function Render(deviceContext: ID3D11DeviceContext; indexCount: integer; worldMatrix, viewMatrix, projectionMatrix: TXMMATRIX;
            texture: ID3D11ShaderResourceView; diffuseColor,lightPosition: array of TXMFLOAT4
							  ): HResult;
...
var
    ...
    cameraBufferDesc: TD3D11_BUFFER_DESC;
    	 ...
    	 lightColorBufferDesc :TD3D11_BUFFER_DESC;
	 lightPositionBufferDesc :TD3D11_BUFFER_DESC;
begin
	...
	lightColorBufferDesc.ByteWidth := sizeof(TLightColorBufferType);
	lightColorBufferDesc.BindFlags := ord(D3D11_BIND_CONSTANT_BUFFER);
	lightColorBufferDesc.CPUAccessFlags := ord(D3D11_CPU_ACCESS_WRITE);
	lightColorBufferDesc.MiscFlags := 0;
	lightColorBufferDesc.StructureByteStride := 0;

	// Create the constant buffer pointer so we can access the pixel shader constant buffer from within this class.
	result := device.CreateBuffer(lightColorBufferDesc, nil, m_lightColorBuffer);
	if (FAILED(Result)) then Exit;

	// Setup the description of the dynamic constant buffer that is in the vertex shader.
	lightPositionBufferDesc.Usage := D3D11_USAGE_DYNAMIC;
	lightPositionBufferDesc.ByteWidth := sizeof(TLightPositionBufferType);
	lightPositionBufferDesc.BindFlags := ord(D3D11_BIND_CONSTANT_BUFFER);
	lightPositionBufferDesc.CPUAccessFlags :=ord( D3D11_CPU_ACCESS_WRITE);
	lightPositionBufferDesc.MiscFlags := 0;
	lightPositionBufferDesc.StructureByteStride := 0;

	// Create the constant buffer pointer so we can access the vertex shader constant buffer from within this class.
	result := device.CreateBuffer(lightPositionBufferDesc, nil, m_lightPositionBuffer);
...
begin
    ...
    m_lightBuffer := nil;
    m_lightColorBuffer:=nil;
    m_lightPositionBuffer :=nil;
...
var
    ...
    dataPtr: ^TMatrixBufferType;
     dataPtr2 :^TLightPositionBufferType;
	 dataPtr3 : ^TLightColorBufferType;
...
begin
    ...
    Result := InitializeShader(device, hwnd, '.\data\light.vs', '.\data\light.ps');
end;
...
function TLightShaderClass.Render(deviceContext: ID3D11DeviceContext; indexCount: integer;
    worldMatrix, viewMatrix, projectionMatrix: TXMMATRIX; texture: ID3D11ShaderResourceView; diffuseColor,lightPosition: array of TXMFLOAT4   ): HResult;
begin
