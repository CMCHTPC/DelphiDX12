...
type
    ...
    private
        ...
        m_Model: TModelClass;
        m_BumpMapShader: TBumpMapShaderClass;
        m_Light: TLightClass;
        FRotation: single;
    private
...
begin
    ...
    Result := m_Model.Initialize(m_Direct3D.GetDevice(), m_Direct3D.GetDeviceContext(), '.\data\cube.txt', '.\data\stone01.dds', '.\data\bump01.dds', '');
    if (Result <> S_OK) then
    begin
    ...
    m_BumpMapShader := TBumpMapShaderClass.Create;
...
begin
    ...
    if (m_BumpMapShader <> nil) then
    begin
        m_BumpMapShader.Shutdown();
        m_BumpMapShader.Free;
        m_BumpMapShader := nil;
    end;
