...
type
    ...
    TFadeShaderClass = class(TObject)
    private
    ...
    private
        ...
        function SetShaderParameters(deviceContext: ID3D11DeviceContext; worldMatrix, viewMatrix, projectionMatrix: TXMMATRIX;
            texture: ID3D11ShaderResourceView; fadeAmount: single): HResult;
...
function TFadeShaderClass.InitializeShader(device: ID3D11Device; hwnd: HWND; vsFilename, psFilename: WideString): HResult;
var
    ...
    fadeBufferDesc: TD3D11_BUFFER_DESC;
begin
    // Initialize the pointers this function will use to null.


    // Compile the vertex shader code.
    Result := D3DCompileFromFile(pwidechar(vsFilename), nil, nil, 'FadeVertexShader', 'vs_5_0',
    ...
    Result := D3DCompileFromFile(pwidechar(psFilename), nil, nil, 'FadePixelShader', 'ps_5_0',
    ...
    fadeBufferDesc.ByteWidth := sizeof(TFadeBufferType);
    fadeBufferDesc.BindFlags := Ord(D3D11_BIND_CONSTANT_BUFFER);
    fadeBufferDesc.CPUAccessFlags := Ord(D3D11_CPU_ACCESS_WRITE);
    fadeBufferDesc.MiscFlags := 0;
    fadeBufferDesc.StructureByteStride := 0;

    // Create the constant buffer pointer so we can access the pixel shader constant buffer from within this class.
    Result := device.CreateBuffer(fadeBufferDesc, nil, m_fadeBuffer);

end;



procedure TFadeShaderClass.ShutdownShader();
begin
    m_fadeBuffer := nil;
...
procedure TFadeShaderClass.OutputShaderErrorMessage(errorMessage: ID3D10Blob; hwnd: HWND; shaderFilename: WideString);
var
...
function TFadeShaderClass.SetShaderParameters(deviceContext: ID3D11DeviceContext; worldMatrix, viewMatrix, projectionMatrix: TXMMATRIX;
    texture: ID3D11ShaderResourceView; fadeAmount: single): HResult;
var
    mappedResource: TD3D11_MAPPED_SUBRESOURCE;
    dataPtr: ^TMatrixBufferType;
    bufferNumber: UINT;
    dataPtr2: ^TFadeBufferType;
begin
...
procedure TFadeShaderClass.RenderShader(deviceContext: ID3D11DeviceContext; indexCount: integer);
begin
...
constructor TFadeShaderClass.Create;
begin
...
destructor TFadeShaderClass.Destroy;
begin
    inherited Destroy;
end;
...
function TFadeShaderClass.Initialize(device: ID3D11Device; hwnd: HWND): Hresult;
begin
    // Initialize the vertex and pixel shaders.
    Result := InitializeShader(device, hwnd, '.\data\fade.vs', '.\data\fade.ps');
end;



procedure TFadeShaderClass.Shutdown();
begin
...
function TFadeShaderClass.Render(deviceContext: ID3D11DeviceContext; indexCount: integer;
    worldMatrix, viewMatrix, projectionMatrix: TXMMATRIX; texture: ID3D11ShaderResourceView; fadeAmount: single): HResult;
begin
