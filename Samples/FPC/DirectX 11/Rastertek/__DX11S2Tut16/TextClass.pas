...
type
	...
	function SetRenderCount(count: integer;deviceContext: ID3D11DeviceContext):HResult;	
    end;
...
begin
    ...
    Result := UpdateSentence(m_sentence1, 'Render Count: ', 20, 20, 0.0, 1.0, 0.0, deviceContext);
    if (Result <> S_OK) then  Exit;
{
    // Initialize the first sentence.
    ...
    // Now update the sentence vertex buffer with the new string information.
    Result := UpdateSentence(m_sentence2, 'Cpu: ', 20, 40, 0.0, 1.0, 0.0, deviceContext);
}
end;
...
function TTextClass.SetRenderCount(count: integer; deviceContext : ID3D11DeviceContext): HResult;
var
countString: ansistring;

begin
	// Setup the render count string.
	countString:='Render Count: '+ IntToStr(count);


	// Update the sentence vertex buffer with the new string information.
	result := UpdateSentence(m_sentence1, countString, 20, 20, 1.0, 1.0, 1.0, deviceContext);
end;
