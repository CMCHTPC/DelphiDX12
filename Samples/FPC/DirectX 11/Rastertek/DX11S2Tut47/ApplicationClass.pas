...
begin
    ...
    Result := m_Model.Initialize(m_Direct3D.GetDevice(), m_Direct3D.GetDeviceContext(), '.\data\sphere.txt', '.\data\blue.dds');
    if (Result <> S_OK) then
    begin
    ...
    Result := m_Bitmap.Initialize(m_Direct3D.GetDevice(), m_Direct3D.GetDeviceContext(), screenWidth, screenHeight, '.\data\mouse.dds', 32, 32);
    if (Result <> S_OK) then
    begin
