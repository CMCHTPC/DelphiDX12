...
type
    ...
    private
        ...
        function SetShaderParameters(deviceContext: ID3D11DeviceContext; worldMatrix, viewMatrix, projectionMatrix: TD3DXMATRIX): HResult;
    ...
    public
        ...
        procedure Shutdown();
        function Render(deviceContext: ID3D11DeviceContext; indexCount: integer; worldMatrix, viewMatrix, projectionMatrix: TD3DXMATRIX): HResult;
    end;
...
begin
    ...
    Result := InitializeShader(device, hwnd, '.\data\color.vs', '.\data\color.ps');
end;
...
function TColorShaderClass.Render(deviceContext: ID3D11DeviceContext; indexCount: integer; worldMatrix, viewMatrix, projectionMatrix: TD3DXMATRIX): HResult;
begin
...
begin
    ...
    Result := D3DX11CompileFromFileW(pWideChar(vsFilename), nil, nil, 'ColorVertexShader', 'vs_5_0', D3D10_SHADER_ENABLE_STRICTNESS, 0, nil, vertexShaderBuffer,
      errorMessage, nil);
    ...
    Result := D3DX11CompileFromFileW(pwidechar(psFilename), nil, nil, 'ColorPixelShader', 'ps_5_0', D3D10_SHADER_ENABLE_STRICTNESS, 0, nil, pixelShaderBuffer,
      errorMessage, nil);
    if (FAILED(Result)) then
    begin
        // If the shader failed to compile it should have writen something to the error message.
...
function TColorShaderClass.SetShaderParameters(deviceContext: ID3D11DeviceContext; worldMatrix, viewMatrix, projectionMatrix: TD3DXMATRIX): HResult;
var
