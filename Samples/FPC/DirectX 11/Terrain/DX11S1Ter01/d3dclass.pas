...
type
    ...
    private
        ...
        m_videoCardDescription: widestring;
        ...
        m_projectionMatrix: TD3DXMATRIX;
        m_worldMatrix: TD3DXMATRIX;
        m_orthoMatrix: TD3DXMATRIX;
    ...
    public
        ...
        function Initialize(screenWidth, screenHeight: integer; vsync: boolean; hwnd: hwnd; fullscreen: boolean; screenDepth, screenNear: single): HResult;
        procedure Shutdown();
        ...
        procedure GetProjectionMatrix(out projectionMatrix: TD3DXMATRIX);
        procedure GetWorldMatrix(out worldMatrix: TD3DXMATRIX);
        procedure GetOrthoMatrix(out orthoMatrix: TD3DXMATRIX);

        procedure GetVideoCardInfo(var cardName: ansistring; var memory: integer);
...
function TD3DClass.Initialize(screenWidth, screenHeight: integer; vsync: boolean; hwnd: hwnd; fullscreen: boolean; screenDepth, screenNear: single): HResult;
var

    factory: IDXGIFactory;
    adapter: IDXGIAdapter;
    adapterOutput: IDXGIOutput;
    numModes, i, numerator, denominator, stringLength: UINT;
...
begin
    ...
    m_videoCardMemory := adapterDesc.DedicatedVideoMemory div 1024 div 1024;

    // Convert the name of the video card to a character array and store it.
    m_videoCardDescription:= adapterDesc.Description;
    (* error := wcstombs_s(&stringLength, m_videoCardDescription, 128, , 128);
      if(error !:= 0)
      begin
      return false;
      end; *)
    ...
    result := D3D11CreateDeviceAndSwapChain(nil, D3D_DRIVER_TYPE_HARDWARE, 0, ord(D3D11_CREATE_DEVICE_DEBUG) , @featureLevel, 1, D3D11_SDK_VERSION, @swapChainDesc, @m_swapChain, @m_device,
      @lTempLevel, @m_deviceContext);
    if (result <> s_OK) then
        Exit;
...
procedure TD3DClass.GetProjectionMatrix(out projectionMatrix: TD3DXMATRIX);
begin
    projectionMatrix := m_projectionMatrix;
end;

procedure TD3DClass.GetWorldMatrix(out worldMatrix: TD3DXMATRIX);
begin
    worldMatrix := m_worldMatrix;
end;

procedure TD3DClass.GetOrthoMatrix(out orthoMatrix: TD3DXMATRIX);
begin
    orthoMatrix := m_orthoMatrix;
end;

procedure TD3DClass.GetVideoCardInfo(var cardName: ansistring; var memory: integer);
begin
    cardName:=ansistring(m_videoCardDescription);
