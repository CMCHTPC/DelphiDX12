...
type
    ...
    public
        ...
        procedure Shutdown();
        function Render(deviceContext: ID3D11DeviceContext; indexCount: integer; worldMatrix, viewMatrix, projectionMatrix: TD3DXMATRIX; texture: ID3D11ShaderResourceView; pixelColor: TD3DXVECTOR4): HResult;
    ...
    private
        ...
        function SetShaderParameters(deviceContext: ID3D11DeviceContext; worldMatrix, viewMatrix, projectionMatrix: TD3DXMATRIX; texture: ID3D11ShaderResourceView; pixelColor: TD3DXVECTOR4): HResult;
...
begin
    ...
    Result := InitializeShader(device, hwnd, '.\data\font.vs', '.\data\font.ps');
end;
...
function TFontShaderClass.Render(deviceContext: ID3D11DeviceContext; indexCount: integer; worldMatrix, viewMatrix, projectionMatrix: TD3DXMATRIX; texture: ID3D11ShaderResourceView; pixelColor: TD3DXVECTOR4): HResult;
begin
...
begin
    ...
    Result := D3DX11CompileFromFileW(pwidechar(vsFilename), nil, nil, 'FontVertexShader', 'vs_5_0', D3D10_SHADER_ENABLE_STRICTNESS, 0, nil, vertexShaderBuffer, errorMessage, nil);
    ...
    Result := D3DX11CompileFromFileW(pwidechar(psFilename), nil, nil, 'FontPixelShader', 'ps_5_0', D3D10_SHADER_ENABLE_STRICTNESS, 0, nil, pixelShaderBuffer, &errorMessage, nil);
    if (FAILED(Result)) then
    begin
        // If the shader failed to compile it should have writen something to the error message.
        if (errorMessage <> nil) then
    ...
    Result := device.CreateInputLayout(@polygonLayout[0], numElements, vertexShaderBuffer.GetBufferPointer(), vertexShaderBuffer.GetBufferSize(), m_layout);
    if (FAILED(Result)) then Exit;
...
function TFontShaderClass.SetShaderParameters(deviceContext: ID3D11DeviceContext; worldMatrix, viewMatrix, projectionMatrix: TD3DXMATRIX; texture: ID3D11ShaderResourceView; pixelColor: TD3DXVECTOR4): HResult;
var
