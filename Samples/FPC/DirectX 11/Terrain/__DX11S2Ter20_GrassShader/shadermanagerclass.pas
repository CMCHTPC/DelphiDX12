...
type
    ...
    private
        ...
        m_FoliageShader: TFoliageShaderClass;
        m_GrassShader: TGrassShaderClass;
    public
        constructor Create;
        ...
        function RenderFoliageShader(deviceContext: ID3D11DeviceContext; vertexCount, instanceCount: integer;
...
begin
    ...
    m_FoliageShader := TFoliageShaderClass.Create;

    // Initialize the foliage shader object.
    Result := m_FoliageShader.Initialize(device, hwnd);
    if (Result <> S_OK) then
        Exit;
...
begin
    ...
    if (m_FoliageShader <> nil) then
    begin
        m_FoliageShader.Shutdown();
        m_FoliageShader.Free;
        m_FoliageShader := nil;
    end;
...
function TShaderManagerClass.RenderFoliageShader(deviceContext: ID3D11DeviceContext; vertexCount, instanceCount: integer;
    viewMatrix, projectionMatrix: TXMMATRIX; texture: ID3D11ShaderResourceView): HResult;
begin
    Result := m_FoliageShader.Render(deviceContext, vertexCount, instanceCount, viewMatrix, projectionMatrix, texture);
end;
