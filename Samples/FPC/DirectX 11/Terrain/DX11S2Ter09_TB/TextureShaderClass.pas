...
type
    ...
    TTextureShaderClass = class(TObject)
    private
    ...
    private
        ...
        function SetShaderParameters(deviceContext: ID3D11DeviceContext; worldMatrix, viewMatrix, projectionMatrix: TXMMATRIX; texture: ID3D11ShaderResourceView): HResult;
    ...
    public
        ...
        procedure Shutdown();
        function Render(deviceContext: ID3D11DeviceContext; indexCount: integer; worldMatrix, viewMatrix, projectionMatrix: TXMMATRIX; texture: ID3D11ShaderResourceView): HResult;
...
function TTextureShaderClass.InitializeShader(device: ID3D11Device; hwnd: HWND; vsFilename, psFilename: widestring): HResult;
var
...
begin
    ...
    Result := D3DCompileFromFile(pwidechar(vsFilename), nil, nil, 'TextureVertexShader', 'vs_5_0', D3D10_SHADER_ENABLE_STRICTNESS, 0, vertexShaderBuffer, errorMessage);
    ...
    Result := D3DCompileFromFile(pwidechar(psFilename), nil, nil, 'TexturePixelShader', 'ps_5_0', D3D10_SHADER_ENABLE_STRICTNESS, 0, pixelShaderBuffer, errorMessage);
    if (FAILED(Result)) then
    begin
        // If the shader failed to compile it should have writen something to the error message.
        if (errorMessage <> nil) then
...
procedure TTextureShaderClass.ShutdownShader();
begin
...
procedure TTextureShaderClass.OutputShaderErrorMessage(errorMessage: ID3D10Blob; hwnd: HWND; shaderFilename: widestring);
var
...
function TTextureShaderClass.SetShaderParameters(deviceContext: ID3D11DeviceContext; worldMatrix, viewMatrix, projectionMatrix: TXMMATRIX; texture: ID3D11ShaderResourceView): HResult;
var
...
procedure TTextureShaderClass.RenderShader(deviceContext: ID3D11DeviceContext; indexCount: integer);
begin
...
constructor TTextureShaderClass.Create;
begin
...
destructor TTextureShaderClass.Destroy;
begin
    inherited Destroy;
end;
...
function TTextureShaderClass.Initialize(device: ID3D11Device; hwnd: HWND): Hresult;
begin
    // Initialize the vertex and pixel shaders.
    Result := InitializeShader(device, hwnd, '.\data\texture.vs', '.\data\texture.ps');
end;



procedure TTextureShaderClass.Shutdown();
begin
...
function TTextureShaderClass.Render(deviceContext: ID3D11DeviceContext; indexCount: integer; worldMatrix, viewMatrix, projectionMatrix: TXMMATRIX; texture: ID3D11ShaderResourceView): HResult;
begin
