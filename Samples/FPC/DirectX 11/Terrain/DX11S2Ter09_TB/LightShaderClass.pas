...
type
    ...
    private
        ...
        function SetShaderParameters(deviceContext: ID3D11DeviceContext; worldMatrix, viewMatrix, projectionMatrix: TXMMATRIX; texture: ID3D11ShaderResourceView;
    ...
    public
        ...
        procedure Shutdown();
        function Render(deviceContext: ID3D11DeviceContext; indexCount: integer; worldMatrix, viewMatrix, projectionMatrix: TXMMATRIX; texture: ID3D11ShaderResourceView;
            lightDirection: TXMFLOAT3; {ambientColor: TXMFLOAT4;} diffuseColor: TXMFLOAT4): HResult;
...
begin
    ...
    {$ELSE}
    // Compile the vertex shader code.
    Result := D3DX11CompileFromFileW(pwidechar(vsFilename), nil, nil, 'LightVertexShader', 'vs_5_0', D3D10_SHADER_ENABLE_STRICTNESS, 0, nil, vertexShaderBuffer, errorMessage, nil);
    {$ENDIF}
    ...
    {$ELSE}
    // Compile the vertex shader code.
    Result := D3DX11CompileFromFileW(pwidechar(psFilename), nil, nil, 'LightPixelShader', 'ps_5_0', D3D10_SHADER_ENABLE_STRICTNESS, 0, nil, pixelShaderBuffer, errorMessage, nil);
    {$ENDIF}
    if (FAILED(Result)) then
    begin
        // If the shader failed to compile it should have writen something to the error message.
...
function TLightShaderClass.SetShaderParameters(deviceContext: ID3D11DeviceContext; worldMatrix, viewMatrix, projectionMatrix: TXMMATRIX; texture: ID3D11ShaderResourceView;
    lightDirection: TXMFLOAT3;{ ambientColor: TXMFLOAT4;} diffuseColor: TXMFLOAT4): HResult;
var
...
function TLightShaderClass.Render(deviceContext: ID3D11DeviceContext; indexCount: integer; worldMatrix, viewMatrix, projectionMatrix: TXMMATRIX; texture: ID3D11ShaderResourceView; lightDirection: TXMFLOAT3;
