...
type
    ...
    private
        ...
        function SetShaderParameters(deviceContext: ID3D11DeviceContext; worldMatrix, viewMatrix, projectionMatrix: TXMMATRIX; texture: ID3D11ShaderResourceView; pixelColor: TXMFLOAT4): HResult;
    ...
    public
        ...
        procedure Shutdown();
        function Render(deviceContext: ID3D11DeviceContext; indexCount: integer; worldMatrix, viewMatrix, projectionMatrix: TXMMATRIX; texture: ID3D11ShaderResourceView; pixelColor: TXMFLOAT4): HResult;
    end;
...
begin
    ...
    Result := D3DCompileFromFile(pwidechar(vsFilename), nil, nil, 'FontVertexShader', 'vs_5_0', D3D10_SHADER_ENABLE_STRICTNESS, 0, vertexShaderBuffer, errorMessage);
    ...
    Result := D3DCompileFromFile(pwidechar(psFilename), nil, nil, 'FontPixelShader', 'ps_5_0', D3D10_SHADER_ENABLE_STRICTNESS, 0, pixelShaderBuffer, errorMessage);
    if (FAILED(Result)) then
    begin
        // If the shader failed to compile it should have writen something to the error message.
        if (errorMessage <> nil) then
...
function TFontShaderClass.SetShaderParameters(deviceContext: ID3D11DeviceContext; worldMatrix, viewMatrix, projectionMatrix: TXMMATRIX; texture: ID3D11ShaderResourceView; pixelColor: TXMFLOAT4): HResult;
var
...
function TFontShaderClass.Render(deviceContext: ID3D11DeviceContext; indexCount: integer; worldMatrix, viewMatrix, projectionMatrix: TXMMATRIX; texture: ID3D11ShaderResourceView; pixelColor: TXMFLOAT4): HResult;
begin
