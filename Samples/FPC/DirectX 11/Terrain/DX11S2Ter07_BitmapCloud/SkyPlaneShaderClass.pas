...
type
    ...
    TSkyBufferType = record
        firstTranslationX: single;
        firstTranslationZ: single;
        secondTranslationX: single;
        secondTranslationZ: single;
        brightness: single;
        padding: TXMFLOAT3;
    end;

    { TSkyPlaneShaderClass }

    TSkyPlaneShaderClass = class(TObject)
    private
    ...
    private
        ...
        function SetShaderParameters(deviceContext: ID3D11DeviceContext; worldMatrix, viewMatrix, projectionMatrix: TXMMATRIX;
            texture: ID3D11ShaderResourceView; texture2: ID3D11ShaderResourceView;
            firstTranslationX, firstTranslationZ, secondTranslationX, secondTranslationZ, brightness: single): HResult;
...
function TSkyPlaneShaderClass.InitializeShader(device: ID3D11Device; hwnd: HWND; vsFilename, psFilename: WideString): HResult;
var
    ...
    skyBufferDesc: TD3D11_BUFFER_DESC;
begin
    // Initialize the pointers this function will use to null.

    // Compile the vertex shader code.
    Result := D3DCompileFromFile(pwidechar(vsFilename), nil, nil, 'SkyPlaneVertexShader', 'vs_5_0',
    ...
    Result := D3DCompileFromFile(pwidechar(psFilename), nil, nil, 'SkyPlanePixelShader', 'ps_5_0', D3D10_SHADER_ENABLE_STRICTNESS,
    ...
    skyBufferDesc.ByteWidth := sizeof(TSkyBufferType);
    skyBufferDesc.BindFlags := Ord(D3D11_BIND_CONSTANT_BUFFER);
    skyBufferDesc.CPUAccessFlags := Ord(D3D11_CPU_ACCESS_WRITE);
    skyBufferDesc.MiscFlags := 0;
    skyBufferDesc.StructureByteStride := 0;

    // Create the constant buffer pointer so we can access the vertex shader constant buffer from within this class.
    Result := device.CreateBuffer(skyBufferDesc, nil, m_skyBuffer);
end;



procedure TSkyPlaneShaderClass.ShutdownShader();
begin
...
procedure TSkyPlaneShaderClass.OutputShaderErrorMessage(errorMessage: ID3D10Blob; hwnd: HWND; shaderFilename: WideString);
var
...
function TSkyPlaneShaderClass.SetShaderParameters(deviceContext: ID3D11DeviceContext; worldMatrix, viewMatrix, projectionMatrix: TXMMATRIX;
    texture: ID3D11ShaderResourceView; texture2: ID3D11ShaderResourceView;
    firstTranslationX, firstTranslationZ, secondTranslationX, secondTranslationZ, brightness: single): HResult;
var
    mappedResource: TD3D11_MAPPED_SUBRESOURCE;
    dataPtr: ^TMatrixBufferType;
    dataPtr2: ^TSkyBufferType;
    bufferNumber: UINT;
begin
...
procedure TSkyPlaneShaderClass.RenderShader(deviceContext: ID3D11DeviceContext; indexCount: integer);
begin
...
constructor TSkyPlaneShaderClass.Create;
begin
...
destructor TSkyPlaneShaderClass.Destroy;
begin
    inherited Destroy;
end;
...
function TSkyPlaneShaderClass.Initialize(device: ID3D11Device; hwnd: HWND): Hresult;
begin
    // Initialize the vertex and pixel shaders.
    Result := InitializeShader(device, hwnd, '.\data\skyplane.vs', '.\data\skyplane.ps');
end;



procedure TSkyPlaneShaderClass.Shutdown();
begin
...
function TSkyPlaneShaderClass.Render(deviceContext: ID3D11DeviceContext; indexCount: integer;
    worldMatrix, viewMatrix, projectionMatrix: TXMMATRIX; texture: ID3D11ShaderResourceView; texture2: ID3D11ShaderResourceView;
    firstTranslationX, firstTranslationZ, secondTranslationX, secondTranslationZ, brightness: single): HResult;
begin
    // Set the shader parameters that it will use for rendering.
    Result := SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, texture, texture2,
        firstTranslationX, firstTranslationZ, secondTranslationX, secondTranslationZ, brightness);
