...
type
    ...
    private
        ...
        m_SkyDomeShader: TSkyDomeShaderClass;
        m_SkyPlaneShader: TSkyPlaneShaderClass;
    public
        constructor Create;
        ...
        function RenderSkyDomeShader(deviceContext: ID3D11DeviceContext; indexCount: integer;
            worldMatrix, viewMatrix, projectionMatrix: TXMMATRIX; apexColor, centerColor: TXMFLOAT4): HResult;

        function RenderSkyPlaneShader(deviceContext: ID3D11DeviceContext; indexCount: integer;
            worldMatrix, viewMatrix, projectionMatrix: TXMMATRIX; texture: ID3D11ShaderResourceView;
            texture2: ID3D11ShaderResourceView; firstTranslationX, firstTranslationZ, secondTranslationX,
            secondTranslationZ, brightness: single): HResult;
    end;
...
begin
    ...
    m_SkyDomeShader := TSkyDomeShaderClass.Create;
    ...
    m_SkyPlaneShader := TSkyPlaneShaderClass.Create;
...
begin
    ...
    if (m_SkyDomeShader <> nil) then
    begin
        m_SkyDomeShader.Shutdown();
        m_SkyDomeShader.Free;
        m_SkyDomeShader := nil;
    end;

    // Release the sky plane shader object.
    if (m_SkyPlaneShader <> nil) then
    begin
        m_SkyPlaneShader.Shutdown();
        m_SkyPlaneShader.Free;
        m_SkyPlaneShader := nil;
    end;
...
function TShaderManagerClass.RenderTerrainShader(deviceContext: ID3D11DeviceContext; indexCount: integer;
    worldMatrix, viewMatrix, projectionMatrix: TXMMATRIX; texture: ID3D11ShaderResourceView; normalMap: ID3D11ShaderResourceView;
...
begin
    Result := m_TerrainShader.Render(deviceContext, indexCount, worldMatrix, viewMatrix, projectionMatrix,
        texture, normalMap, lightDirection, diffuseColor);
end;



function TShaderManagerClass.RenderSkyDomeShader(deviceContext: ID3D11DeviceContext; indexCount: integer;
    worldMatrix, viewMatrix, projectionMatrix: TXMMATRIX; apexColor, centerColor: TXMFLOAT4): HResult;
begin
...
function TShaderManagerClass.RenderSkyPlaneShader(deviceContext: ID3D11DeviceContext; indexCount: integer;
...
begin
    Result := m_SkyPlaneShader.Render(deviceContext, indexCount, worldMatrix, viewMatrix, projectionMatrix,
        texture, texture2, firstTranslationX, firstTranslationZ, secondTranslationX, secondTranslationZ, brightness);
end;
