...
type
    ...
    public
        ...
        function Render(Direct3D: TD3DClass; ShaderManager: TShaderManagerClass; worldMatrix, viewMatrix, orthoMatrix: TXMMATRIX): HResult;
...
begin
    ...
    Result := m_Font1.Initialize(Direct3D.GetDevice(), Direct3D.GetDeviceContext(), '.\data\font\font01.txt', '.\data\font\font01.tga', 32.0, 3);
    ...
    Result := m_FpsString.Initialize(Direct3D.GetDevice(), Direct3D.GetDeviceContext(), screenWidth, screenHeight, 16, False, m_Font1, 'Fps: 0', 10, 50, 0.0, 1.0, 0.0);
    if (Result <> S_OK) then Exit;
    ...
    videoCard := 'Video Card: ' + videoCard;
    memoryString := 'Video Memory: ' + IntToStr(videoMemory) + ' MB';
    ...
    Result := m_VideoStrings[0].Initialize(Direct3D.GetDevice(), Direct3D.GetDeviceContext(), screenWidth, screenHeight, 256, False, m_Font1, videoCard, 10, 10, 1.0, 1.0, 1.0);
    if (Result <> S_OK) then Exit;

    Result := m_VideoStrings[1].Initialize(Direct3D.GetDevice(), Direct3D.GetDeviceContext(), screenWidth, screenHeight, 32, False, m_Font1, memoryString, 10, 30, 1.0, 1.0, 1.0);
    if (Result <> S_OK) then Exit;
    ...
    Result := m_PositionStrings[0].Initialize(Direct3D.GetDevice(), Direct3D.GetDeviceContext(), screenWidth, screenHeight, 16, False, m_Font1, 'X: 0', 10, 310, 1.0, 1.0, 1.0);
    if (Result <> S_OK) then Exit;

    Result := m_PositionStrings[1].Initialize(Direct3D.GetDevice(), Direct3D.GetDeviceContext(), screenWidth, screenHeight, 16, False, m_Font1, 'Y: 0', 10, 330, 1.0, 1.0, 1.0);
    if (Result <> S_OK) then Exit;

    Result := m_PositionStrings[2].Initialize(Direct3D.GetDevice(), Direct3D.GetDeviceContext(), screenWidth, screenHeight, 16, False, m_Font1, 'Z: 0', 10, 350, 1.0, 1.0, 1.0);
    if (Result <> S_OK) then Exit;

    Result := m_PositionStrings[3].Initialize(Direct3D.GetDevice(), Direct3D.GetDeviceContext(), screenWidth, screenHeight, 16, False, m_Font1, 'rX: 0', 10, 370, 1.0, 1.0, 1.0);
    if (Result <> S_OK) then Exit;

    Result := m_PositionStrings[4].Initialize(Direct3D.GetDevice(), Direct3D.GetDeviceContext(), screenWidth, screenHeight, 16, False, m_Font1, 'rY: 0', 10, 390, 1.0, 1.0, 1.0);
    if (Result <> S_OK) then Exit;

    Result := m_PositionStrings[5].Initialize(Direct3D.GetDevice(), Direct3D.GetDeviceContext(), screenWidth, screenHeight, 16, False, m_Font1, 'rZ: 0', 10, 410, 1.0, 1.0, 1.0);
    if (Result <> S_OK) then Exit;
    ...
    Result := m_RenderCountStrings[0].Initialize(Direct3D.GetDevice(), Direct3D.GetDeviceContext(), screenWidth, screenHeight, 32, False, m_Font1, 'Polys Drawn: 0', 10, 260, 1.0, 1.0, 1.0);
    if (Result <> S_OK) then Exit;

    m_RenderCountStrings[1] := TTextClass.Create;
    Result := m_RenderCountStrings[1].Initialize(Direct3D.GetDevice(), Direct3D.GetDeviceContext(), screenWidth, screenHeight, 32, False, m_Font1, 'Cells Drawn: 0', 10, 280, 1.0, 1.0, 1.0);
    if (Result <> S_OK) then Exit;

    m_RenderCountStrings[2] := TTextClass.Create;
    Result := m_RenderCountStrings[2].Initialize(Direct3D.GetDevice(), Direct3D.GetDeviceContext(), screenWidth, screenHeight, 32, False, m_Font1, 'Cells Culled: 0', 10, 300, 1.0, 1.0, 1.0);
...
function TUserInterfaceClass.Render(Direct3D: TD3DClass; ShaderManager: TShaderManagerClass; worldMatrix, viewMatrix, orthoMatrix: TXMMATRIX): HResult;
var
    i: integer;
begin
